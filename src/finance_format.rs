use super::bin_format::{BinReader, BinWriter};
use super::csv_format::{CsvReader, CsvWriter};
use super::error::ParsError;
use super::finance_data::*;
use super::text_format::{TextReader, TextWriter};
...
impl<In: Read> FinReader<In> {
    ...
    pub fn read_fin_data(&mut self) -> Result<Option<FinanceData>, ParsError> {
        match self {
            Self::Csv(csv_reader) => csv_reader.read_fin_data(),
            Self::Text(text_reader) => text_reader.read_fin_data(),
            Self::Bin(bin_reader) => bin_reader.read_fin_data(),
...
impl<Out: Write> FinWriter<Out> {
    ...
    pub fn write_fin_data(&mut self, fin_data: &FinanceData) -> Result<(), ParsError> {
        match self {
            Self::Csv(csv_writer) => csv_writer.write_fin_data(fin_data),
            Self::Text(text_writer) => text_writer.write_fin_data(fin_data),
            Self::Bin(bin_writer) => bin_writer.write_fin_data(fin_data),
            Self::Unsupported(err) => {
                return Err(ParsError::WrongFormat(err.to_owned()));
            }
        }
